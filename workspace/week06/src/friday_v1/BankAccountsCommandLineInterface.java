...
public class BankAccountsCommandLineInterface {
	...
	public BankAccountsCommandLineInterface(Scanner input, PrintStream output,Bank bank) {
		this.mActiveAccount = null;
		this.mInput = input;
		this.mOutput = output;
		this.mBank = bank;
	...
	public BankAccountsCommandLineInterface(BankAccount account,Bank bank) {
		...
		while (true) {
			...
			case "create_bank_account": {
				...
				try {
					setActiveAccount(new BankAccount(firstName[0], lastName[0], age[0], balance[0], interest[0],
							interestType[0]));
					mBank.addAccount(mActiveAccount);
			...
			case "show_history": {
				if (mActiveAccount != null) {
					mActiveAccount.printAccountInformation(mOutput);
				} else {
			...
			case "calculate_amount": {
				if (mActiveAccount != null) {
					...
					while (!safeIntInput("Monate:", months, 0)) {
						mOutput.println("Fehler.");
					}
					try {
						mActiveAccount.calculateAmount(months[0]);
			...
			case "transfer_money": {
				if (mActiveAccount != null) {
					...
					BankAccount account = selectAccount(accountID);
					try {
						mActiveAccount.transfer(account,sum[0]);
					} catch (friday_v1.InputException e) {
						mOutput.println("Fehler.");
					} catch (friday_v1.InsufficientFundsException e) {
						mOutput.println("keine geld");
	...
	public void setAccount(int []accountID){
		while(!safeIntInput("Eintreten ID:",accountID,0)){
			mOutput.println("Fehler.");
			mOutput.println("Eintrere ein korrekt ID.");
		}
		setActiveAccount(mBank.getAccount(accountID[0]));
	}
	
	public BankAccount selectAccount(int []accountID){
		while(!safeIntInput("Eintreten ID:",accountID,0)){
			mOutput.println("Fehler.");
			mOutput.println("Eintrere ein korrekt ID.");
		}
		return mBank.getAccount(accountID[0]);
	}
